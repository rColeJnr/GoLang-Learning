...
import (
	"bufio" // buffered I/O that works on any value that satisfies the io.Reader/Writer interace
	"fmt"
	"io" // Low level io funcs
	"io/ioutil"
	"log"
	"os"
	"path/filepath"
...
func filenamesFromCommandLine() (inFilename string, outFilename string, err error) {
	if len(os.Args) > 1 && (os.Args[1] == "-h" || os.Args[1] == "--help") {
		err = fmt.Errorf("usage: %s [<] infile.xtx [>] outfile.txt", filepath.Base(os.Args[0]))
		return "", "", err
	}
...
func americanise(inFile io.Reader, outFile io.Writer) (err error) {
	...
	for !eof {
		...
		if err == io.EOF {
			...
			return err // finish immediately for real erros
		}
...
func makeReplacerFunction(file string) (func(string) string, error) {
	...
	return func(s string) string {
		if usWord, found := usForBritish[s]; found {
			return usWord
		}
		return s
	}, nil
